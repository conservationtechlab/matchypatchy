...
If fso.FileExists(pythonwPath) Then
  cmd = """" & pythonwPath & """ """ & scriptDir & "\matchypatchy_package\src\matchypatchy\__main__.py" & """"
ElseIf fso.FileExists(pythonPath) Then
  ' fallback to python (will show a console if used)
  cmd = """" & pythonPath & """ """ & scriptDir & "\matchypatchy_package\src\matchypatchy\__main__.py" & """"
Else
  ' final fallback: try Python launcher on PATH (pyw for GUI, py if you want console)
  cmd = "pyw -3 """ & scriptDir & "\matchypatchy_package\src\matchypatchy\__main__.py" & """"
End If
